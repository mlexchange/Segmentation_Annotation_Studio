...
ensure_frontend_runtime() {
  local node_major=""
  if command -v npm >/dev/null 2>&1 && command -v node >/dev/null 2>&1 && can_run_npm "$(command -v npm)"; then
  ...
  echo -e "${RED}Error: npm / Node.js 20+ was not found on PATH.${NC}"
...
# Restrict to the owner regardless of umask — this file holds the Tiled API key.
...
# blank (fresh install) OR matches the fingerprint of the retired legacy key.
...
LEGACY_TILED_KEY_SHA256="ede2e5afabfcbd9007b3d45a736b8b946b43bfea3df8636662a43680e4cf2e00"
CURRENT_TILED_KEY_SHA256="$(printf '%s' "${TILED_API_KEY:-}" | "$PYTHON" -c 'import hashlib, sys; print(hashlib.sha256(sys.stdin.buffer.read()).hexdigest())')"
if [ -z "${TILED_API_KEY// }" ] || [ "$CURRENT_TILED_KEY_SHA256" = "$LEGACY_TILED_KEY_SHA256" ]; then
...
ensure_ml_env() {
  ...
  if "$PYTHON" -c "import torch" >/dev/null 2>&1; then
    echo -e "${GREEN}    torch already installed — Train tab (DINOv3 + LoRA) available.${NC}"
  else
  ...
  if "$PYTHON" -c "import dlsia" >/dev/null 2>&1; then
    ...
    echo -e "${CYAN}==> Installing dlsia for the Train tab's TUNet model family...${NC}"
    if uv pip install --python "$PYTHON" "dlsia>=0.3"; then
  ...
  # Migrate any DINOv3 checkpoint dropped at the repo root into the configured
  # models directory, so the Train tab's checkpoint picker finds it.
  local models_dir="${DINO_MODELS_DIR:-${LOCAL_DATA_ROOT:-$HOME/data}/models/dinov3}"
  models_dir="${models_dir/#\~/$HOME}"
  mkdir -p "$models_dir" 2>/dev/null || true
  shopt -s nullglob
  local moved=0
  for f in "$SCRIPT_DIR"/dinov3_*_pretrain_*.pth; do
    mv "$f" "$models_dir/" && moved=1
  done
  shopt -u nullglob
  if [ "$moved" = "1" ]; then
    echo -e "${GREEN}    Moved DINOv3 checkpoint(s) from repo root to ${models_dir}${NC}"
  fi
