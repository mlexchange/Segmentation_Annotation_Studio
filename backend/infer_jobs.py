...
Runs a saved fine-tuned run (either model family) over requested slices,
producing editable polygon annotations, a colourised overlay preview, and an
optional push of the raw label maps into Tiled (reusing the existing
mask-sync writer). Progress/cancellation share the same :mod:`export_jobs`
registry as training and export jobs.
...
def run_infer_job(jid: str, request: InferRequest) -> None:
    ...
    try:
        ...
        if config["model_family"] == "dinov3_lora":
            import dino_runtime as fam

            backbone, head = fam.load_model(
                config["model_config"],
                config["hyperparams"],
                adapter_state,
                n_classes,
