...
If a ``_studio_shapes.json`` sidecar is present alongside the COCO split, it is
preferred and gives a lossless (exact original vector shape) import. NOTE: as of
this writing, no export path in this codebase (see coco_export.py) writes that
sidecar — every dataset this app itself exports is imported via the fallback
path below, which reconstructs PolygonShapes from segmentation_poly or RLE
contours. That reconstruction is lossy: original rectangle/ellipse/brush shapes
become generic polygons, and exact vertex positions are not preserved. The
sidecar format is supported here for forward-compatibility (e.g. a future
export version, or an external tool that writes one) but is not yet produced.
"""
