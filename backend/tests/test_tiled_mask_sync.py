...
from tiled_mask_sync import (
    ExistingMasksUnreadable,
    _read_existing_masks,
    _validate_safe_class_names,
    build_mask_volumes,
    merge_mask_volumes,
    write_masks_to_tiled,
)
...
def test_unreadable_existing_masks_fail_closed() -> None:
    """A transient read error must abort rather than silently replacing masks."""
    ...
    with pytest.raises(ExistingMasksUnreadable, match="could not read existing"):
        _read_existing_masks(UnreadableContainer())


def test_sanitized_class_key_collisions_are_rejected() -> None:
    """Distinct labels must not overwrite one another after key sanitization."""
    with pytest.raises(ValueError, match="same Tiled key"):
        _validate_safe_class_names(["cell/wall", "cell?wall"])
...
class _FakeMaskContainer:
    def __init__(self, arrays: dict[str, np.ndarray], metadata: dict, fail_once: str | None = None) -> None:
        self.arrays = {key: _FakeArray(value) for key, value in arrays.items()}
    ...
    def write_array(self, data, *, key: str, dims, metadata: dict) -> None:
        if self.fail_once == key:
            self.fail_once = None
            raise RuntimeError(f"failed writing {key}")
        self.arrays[key] = _FakeArray(np.asarray(data).copy())


class _FakeRoot:
    def __init__(self, container: _FakeMaskContainer) -> None:
        self.container = container

    def __getitem__(self, key: str) -> _FakeMaskContainer:
        if key != "image__masks":
            raise KeyError(key)
        return self.container


def test_mask_write_failure_restores_existing_container(monkeypatch) -> None:
    """A partial replacement failure must restore the prior masks and metadata."""
    old_semantic = np.ones((1, 2, 2), dtype=np.uint8)
    old_cell = np.full((1, 2, 2), 255, dtype=np.uint8)
    old_metadata = {
        "slice_indices": [0],
        "legend": [{"id": 1, "name": "Cell", "color": "#ff0000"}],
        "slice_updated_at": {"0": "old"},
        "marker": "preserve-me",
    }
    container = _FakeMaskContainer(
        {"semantic": old_semantic, "Cell": old_cell},
        old_metadata,
        fail_once="Wall",
    )
    root = _FakeRoot(container)
    monkeypatch.setattr("tiled_mask_sync.get_tiled_client", lambda uri, key=None: root)

    new = {
        "semantic": np.full((1, 2, 2), 2, dtype=np.uint8),
        "class_vols": {"Wall": np.full((1, 2, 2), 255, dtype=np.uint8)},
        "slice_indices": [0],
        "legend": [{"id": 1, "name": "Wall", "color": "#00ff00"}],
        "updated_indices": [0],
    }

    with pytest.raises(RuntimeError, match="failed writing Wall"):
        write_masks_to_tiled("image", None, new, [])

    assert container.metadata == old_metadata
    assert set(container.arrays) == {"semantic", "Cell"}
    assert np.array_equal(container.arrays["semantic"].data, old_semantic)
    assert np.array_equal(container.arrays["Cell"].data, old_cell)
