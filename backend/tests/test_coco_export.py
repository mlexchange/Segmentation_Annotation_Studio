...
def test_resolve_split_single_slice_goes_to_train() -> None:
    """A single annotated slice must land in train, not be floored out of every split.

    Regression for a bug where the default 0.8/0.1/0.1 auto-split ratios, applied
    to exactly one slice, floored the train and valid shares to 0 and dumped the
    only annotated slice into test -- so annotating and saving one slice then
    training produced "No annotated slices to train on" even though data existed.
    """
    from coco_export import _resolve_split

    resolved = _resolve_split(["0"], {}, {"ratios": [0.8, 0.1, 0.1], "seed": 1234})
    assert resolved["0"] == "train"


def test_resolve_split_never_starves_train_when_ratio_nonzero() -> None:
    """For any small slice count, a non-zero train ratio always yields >=1 train slice."""
    from coco_export import _resolve_split

    for n in range(1, 10):
        keys = [str(i) for i in range(n)]
        resolved = _resolve_split(keys, {}, {"ratios": [0.8, 0.1, 0.1], "seed": 1234})
        assert list(resolved.values()).count("train") >= 1, f"n={n} produced no train slices"


def test_resolve_split_respects_explicit_zero_train_ratio() -> None:
    """An explicit 0 train ratio (e.g. a pure validation/test export) is not overridden."""
    from coco_export import _resolve_split

    resolved = _resolve_split(["0", "1"], {}, {"ratios": [0.0, 0.5, 0.5], "seed": 1234})
    assert "train" not in resolved.values()
