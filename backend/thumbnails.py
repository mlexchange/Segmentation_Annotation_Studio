...
# A thumbnail only needs a few hundred pixels per side. Requesting a strided
# slice for anything larger than this means a gigapixel array is never fully
# materialized in process memory just to render a 256px preview.
_MAX_SOURCE_DIM = 4096


def _read_downsampled(array_node: Any) -> np.ndarray:
    """Read *array_node*, striding server-side if it exceeds ``_MAX_SOURCE_DIM``.

    Only the leading two (H, W) axes are strided — a trailing channel axis is
    read in full so colour data is not corrupted.
    """
    shape = tuple(int(d) for d in (getattr(array_node, "shape", None) or ()))
    if len(shape) < 2:
        return np.asarray(array_node.read())

    stride = max(1, -(-max(shape[0], shape[1]) // _MAX_SOURCE_DIM))
    if stride == 1:
        return np.asarray(array_node.read())

    index = tuple(
        slice(None, None, stride) if axis < 2 else slice(None)
...
def render_thumbnail(node: Any, size: int = 256) -> bytes | None:
    ...
    arr = np.squeeze(_read_downsampled(array_node))
