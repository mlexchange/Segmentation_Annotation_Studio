...
def _resolve_split(
    ...
    if auto_keys:
        ...
        # Floor rounding can zero out an already-tiny train share (e.g. a single
        # annotated slice with the default 0.8/0.1/0.1 ratios rounds to 0 train,
        # dumping the only sample into test). Guarantee at least one train slice
        # whenever the ratio isn't explicitly 0, so annotating one slice is enough
        # to train on.
        if n_train == 0 and n > 0 and ratios[0] > 0:
            n_train = 1
