...
(dlsia TUNet segmentation, or the dlsia/autoencoder denoiser family), sharing
data preparation, the generic training loop, and run persistence via
:mod:`train_common`. Progress/cancellation are reported through the existing
:mod:`export_jobs` registry — the frontend polls the same
``GET /api/export/status/{job_id}`` route already used by exports and
mask-sync jobs.

DINOv3 LoRA fine-tuning is out of scope here (see Phase 5.5).

Two *tasks* are dispatched from here, distinguished by ``TrainRequest.task``:

* ``"segmentation"`` — annotation-driven, via
  :func:`train_common.prepare_datasets` and
  :func:`train_common.run_training_loop`.
* ``"denoising"`` — self-supervised (Noise2Noise / Noise2Void / autoencoder),
  via :mod:`denoise_train`'s raw-slice samplers and its own regression loop.
...
def _apply_parent_architecture(parent_config: dict, request: TrainRequest) -> None:
    ...
    ``train_common.build_family``'s dispatch.
    """
