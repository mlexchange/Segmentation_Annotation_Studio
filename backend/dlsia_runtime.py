"""dlsia TUNet model building for the Train tab (second model family).

Unlike the DINOv3 LoRA path, a TUNet trains from scratch — no pretrained
checkpoint, no license restriction beyond dlsia's own (BSD). dlsia is an
optional dependency (``backend/pyproject.toml``'s ``ml`` extra, alongside
``torch``) — this module is import-safe without it installed; only
...
model class (and its own save/load helpers) and drives training through the
same generic loop as the DINOv3 family (``train_common.run_training_loop``).
"""
