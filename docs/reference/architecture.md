...
=== "Development"
    ...
    `start_all.sh` launches Tiled (8010), the backend (8002), the Vite dev server
    (5173), and—when available—the documentation site (8000). Every service is
    explicitly bound to host loopback. Vite proxies every `/api` request to the
    backend, so the frontend uses an empty `API_BASE` and stays same-origin.
...
=== "Production (Docker)"
    ...
    image. Compose publishes port 8002 on host loopback only. Although Uvicorn
    listens on the container's internal network interface, it is not published
    on other host interfaces.
