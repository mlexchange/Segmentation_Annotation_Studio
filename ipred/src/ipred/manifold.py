...
from ipred.cache import TTLCache
...
def sample_inducing_points(
    ...
    *,
    ...
    Args:
        float_stack: Feature bank HxWxC.
        feature_id: Owning feature bank id (stored on the sample).
    ...
    """
    feats = np.asarray(float_stack, dtype=np.float32)
    if feats.ndim != 3:
    ...
    return ManifoldSample(job_id=feature_id, points=points, heatmap=heatmap, meta=meta)
