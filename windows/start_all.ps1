...
function Ensure-FrontendRuntime {
  ...
  if ($npmCommand -and $nodeCommand) {
    $nodeMajorText = & node -p "process.versions.node.split('.')[0]" 2>$null
    $nodeMajor = 0
    if (($LASTEXITCODE -eq 0) -and [int]::TryParse([string]$nodeMajorText, [ref]$nodeMajor) -and ($nodeMajor -ge 20)) {
      return
    }
  }
  Err "Error: npm / Node.js 20+ was not found on PATH."
...
# ---------------------------------------------------------------------------
# Restrict .env to the current user only (best-effort) - it holds the Tiled API
# key. icacls failures (e.g. non-NTFS volumes) are non-fatal.
function Protect-EnvFile([string]$Path) {
  try {
    icacls $Path /inheritance:r /grant:r "$($env:USERNAME):(R,W)" *>$null
  } catch {
    Warn "    Could not restrict permissions on $Path"
  }
}
...
# Generate a strong Tiled API key when it is blank (fresh install) or matches the
# fingerprint of the retired legacy key. Persist it to backend\.env and export it.
function Ensure-TiledKey {
  $legacyKeySha256 = 'ede2e5afabfcbd9007b3d45a736b8b946b43bfea3df8636662a43680e4cf2e00'
  $current = $env:TILED_API_KEY
  $isLegacyKey = $false
  if (-not [string]::IsNullOrWhiteSpace($current)) {
    $sha256 = [System.Security.Cryptography.SHA256]::Create()
    try {
      $currentBytes = [System.Text.Encoding]::UTF8.GetBytes($current)
      $currentHash = [System.BitConverter]::ToString($sha256.ComputeHash($currentBytes)).Replace('-', '').ToLowerInvariant()
      $isLegacyKey = ($currentHash -eq $legacyKeySha256)
    } finally {
      $sha256.Dispose()
    }
    if (-not $isLegacyKey) { return }
  }
