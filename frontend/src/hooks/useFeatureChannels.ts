...
import { ipredChannelUrl, ipredPreprocess, listIpredModules, openIpredSession } from '@/lib/ipredApi';
...
export function useFeatureChannels({
  ...
  useEffect(() => {
    ...
    listIpredModules()
      .then((modules) => {
        if (cancelled) return;
        const slimsam = modules.find((m) => m.id === 'slimsam');
        setSamAvailable(!!slimsam?.ready);
      })
      .catch(() => {
  ...
  }, [source, kind, sliceIndex, computing, preferredCompositionId, ensureSession]);
