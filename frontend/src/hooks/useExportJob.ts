/**
 * useExportJob — drive a background job (COCO/mask export, iPred batch
 * train/apply, or the Train tab's train/infer/batch-probe jobs) and stream
 * its progress.
 *
 * POSTs to a job-starting route (which returns a job_id), then polls
...
/**
 * Drives a background job: returns the live job `state`, a `start` action,
...
export function useExportJob(persistKey?: string) {
  ...
  return {
    ...
    start,
    startMaskSync,
    startIpredBatchTrain,
    startIpredBatchApply,
