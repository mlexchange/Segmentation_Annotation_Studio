...
interface ClassReplaceEntry {
  kind: 'classReplace';
  sourceKey: string;
  before: AnnotationClass[];
  after: AnnotationClass[];
}
...
type Entry = ClassDeleteEntry | ClassReplaceEntry | null;
...
/** Tag the next tracked annotation snapshot replacement with its paired class lists. */
export function markClassReplace(
  sourceKey: string,
  before: AnnotationClass[],
  after: AnnotationClass[],
) {
...
export function undo() {
  ...
  } else if (entry?.kind === 'classReplace' && entry.sourceKey === currentSourceKey()) {
    useClassStore.getState().setClasses(entry.before);
  }
  redoOps.push(entry);
}
...
export function redo() {
  ...
  if (entry?.kind === 'classDelete' && entry.sourceKey === currentSourceKey()) {
    ...
    useClassStore.getState().setClasses(entry.after);
  }
  undoOps.push(entry);
}
