/**
 * useTrainRuns — saved fine-tune runs (dlsia_tunet segmentation + the
 * dlsia_denoiser family), for the inference run-picker. Invalidate
 * `['trainRuns']` after a training job completes so a freshly-saved run shows
...
export interface TrainRun {
  run_id: string;
  // Discriminator literals from backend/schemas.py's ModelConfig union.
  model_family: 'dlsia_tunet' | 'dlsia_denoiser';
   ...
   * choice at predict time. It's surfaced (RunsPanel) purely so two runs that
   * expect different input can't look identical.
   */
