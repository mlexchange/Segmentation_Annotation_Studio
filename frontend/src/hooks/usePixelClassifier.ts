...
export function usePixelClassifier({
  ...
  const [treeView, setTreeView] = useState<ClfTreeView | null>(null);
  ...
  const publishProbaPreview = useCallback(
    async (raw: Blob, threshold: number) => {
  ...
  useEffect(() => {
    ...
    revokePredict();
    setError(null);
  }, [featureJobId, resetKey, preferredFeatureSetupId, preferredCompositionId, revokePredict]);
  ...
  const ensureFeatureBank = useCallback(async (): Promise<string> => {
    ...
    if (!compositionId) {
      throw new Error('Select a composition on the Ipred tab first.');
    }
  ...
  const train = useCallback(
    async (shapes: Shape[]) => {
      if (training) return;
      if (!(preferredCompositionId || preferredFeatureSetupId)) {
      ...
      try {
        ...
        const result: ClfTrainResult = {
          ...
          featureSetupId: preferredCompositionId || preferredFeatureSetupId,
        };
        setModel(result);
        setTreeView(null);
        setTreeIndex(0);
  ...
  const loadTree = useCallback(async (index: number) => {
    // Tree inspection not yet exposed on ipred; keep index for UI.
    setTreeIndex(index);
  }, []);
  ...
  const predict = useCallback(
    async (_shapes: Shape[]) => {
      ...
      try {
        ...
        const [commitBlob, statusBlob] = await Promise.all([
          commitRes.blob(),
          statusRes.blob(),
        ]);
  ...
  return {
    ...
    canTrainWithoutJob:
      !!(preferredCompositionId || preferredFeatureSetupId) && !!source && !!kind,
  };
}
