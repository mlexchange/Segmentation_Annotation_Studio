...
describe('magicwand', () => {
  it('global mode selects all similar regions at once', () => {
    ...
    const polys = magicSelect(field, 9, 9, { toleranceFrac: 0.2, mode: 'global', blur: 0, minRegion: 8 });
    expect(polys.length).toBe(2);
  });

  it('contiguous mode selects only the clicked region', () => {
    const field = twoBlocks();
    const polys = magicSelect(field, 9, 9, { toleranceFrac: 0.2, mode: 'contiguous', blur: 0, minRegion: 8 });
  ...
  it('minRegion drops specks', () => {
    ...
    const polys = magicSelect({ gw, gh, scale: 1, gray }, 8, 8, {
      toleranceFrac: 0.3, mode: 'global', blur: 0, minRegion: 12,
    });
    expect(polys.length).toBe(1);
  });
  ...
  it('scales polygon coordinates back to image space', () => {
    ...
    const polys = magicSelect(field, 36, 36, { toleranceFrac: 0.2, mode: 'contiguous', blur: 0, minRegion: 8 });
    expect(polys.length).toBe(1);
