 ...
 *
 * Keep in sync with `TunetHyperParams` in backend/schemas.py — that remains the
 * authority; this only exists to fail earlier and more legibly.
 *
 * dlsia TUNet only — DINOv3 LoRA is deferred (see Phase 5.5 in the integration
 * plan), so there is only one family to constrain here.
 */

export type TrainFamily = 'dlsia_tunet';
