 ...
 * - The local launchers set this to their selected `mkdocs serve` address.
 * - It stays empty in builds that do not deploy documentation, hiding the link
 *   instead of sending users to an unrelated service on their own machine.
 * - Set `VITE_DOCS_URL` at build time to point at a deployed docs site
 *   (e.g. `https://docs.example.com`).
 */
export const DOCS_URL = import.meta.env.VITE_DOCS_URL?.trim() || '';
