...
/** Sidebar / floating layer stack controls. */
export default function LayersPanel({
  ...
  const toggleClassVis = useClassStore((s) => s.toggleVisibility);
  ...
  return (
    ...
    >
      ...
      <p className="text-[9px] leading-snug text-gray-400 -mt-0.5">
        Toggle function groups and per-class visibility. Probability stays an overlay so
        annotations and predictions can show together.
      </p>
        ...
        <div className="flex flex-col gap-0.5 border-t border-gray-100 pt-1.5">
          ...
          {classes.map((c) => (
            <button
              key={`ann-${c.classId}`}
              type="button"
              onClick={() => toggleClassVis(c.classId)}
              className="flex items-center gap-1.5 rounded px-1 py-0.5 text-left text-[10px] hover:bg-sky-50"
            >
              {c.isVisible ? (
              ...
              <span
                className="inline-block h-2 w-2 shrink-0 rounded-sm"
                style={{ background: c.color, opacity: c.isVisible ? 1 : 0.3 }}
              />
              <span className={c.isVisible ? 'text-gray-800' : 'text-gray-400'}>{c.label}</span>
            </button>
          ))}
        </div>
      )}
