...
export default function CapabilityBanner({ capability }: CapabilityBannerProps) {
      ...
      <div className="flex items-start gap-2 rounded-lg border border-amber-600/50 bg-amber-900/20 px-3 py-2.5 text-sm text-amber-200">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-amber-200/80">
            torch isn't installed. Run this app via <code className="font-mono">start_all.sh</code> on a
            machine where it can install ML dependencies — the Docker image intentionally omits them.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-xs text-slate-300">
      ...
      <span>
        DINOv3 checkpoints:{' '}
        <span className={capability.dinov3.checkpoints.length ? 'text-slate-200' : 'text-amber-300'}>
          {capability.dinov3.checkpoints.length || 'none found'}
        </span>
      </span>
      <span>
        dlsia (TUNet):{' '}
