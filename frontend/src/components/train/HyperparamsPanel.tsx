/**
 * HyperparamsPanel — training hyperparameters. Shared fields (epochs, lr,
 * batch size, image size, flip augment) apply to both model families;
 * DINOv3 additionally exposes LoRA rank/alpha, dlsia TUNet exposes
 * depth/base_channels/growth_rate. Collapsed behind <details> — sensible
 * defaults are supplied by the backend schema, so most users never open it.
 */
import type { ModelFamily } from './ModelPickerPanel';
...
export default function HyperparamsPanel({
  modelFamily, values, onChange, runName, onRunNameChange,
  onEstimateBatch, onCancelEstimate, estimatingBatch, batchEstimateNote, estimateDisabledReason,
}: HyperparamsPanelProps) {
  const sizeLimits = IMAGE_SIZE_CONSTRAINTS[modelFamily];
  const sizeError = validateImageSize(
    modelFamily, values.image_size, values.tiling ? 'Patch size' : 'Image size',
  );
      ...
      <div className="grid grid-cols-2 gap-3 px-3 pb-3 pt-1 sm:grid-cols-3">
        ...
        <div>
          {/* Same number, two meanings: the tile window when tiling, or the square
              the whole slice is squashed into when not. The range is shown because
              the bounds differ per family and are easy to trip over. */}
        ...
        {modelFamily === 'dinov3_lora' ? (
          <>
            <div>
              ...
              <input
                type="number" min={1} value={values.lora_rank}
                onChange={(e) => onChange({ lora_rank: Number(e.target.value) })} className={inputClass}
              />
            </div>
            <div>
              <label className={labelClass}>LoRA alpha</label>
              <input
                type="number" min={1} value={values.lora_alpha}
                onChange={(e) => onChange({ lora_alpha: Number(e.target.value) })} className={inputClass}
              />
            </div>
          </>
        ) : (
