...
export default function InferencePanel({
  ...
  // so a stale count from a previous job never lingers.
  ...
  const handleCancelInference = () => {
    ...
    if (job.jobId) void fetch(`${API_BASE}/api/export/cancel/${job.jobId}`, { method: 'POST' });
  };
  ...
  // the run list settles, but a still-running job keeps going server-side and
  // this hook already reattached to it (see useExportJob's persistKey). Gating
  // the whole section on selectedRunId used to hide that reattached job behind
  // "select a run", making a still-running (or already-finished) job look like
  // it vanished.
