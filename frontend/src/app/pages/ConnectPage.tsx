...
export default function ConnectPage() {
  ...
  // null = nothing explicitly selected yet; '' = the granted root itself was
  // explicitly chosen as the dataset folder. Both are falsy as plain strings,
  // which previously made "Use this root as dataset folder" a no-op — see canConnect.
  ...
  const canConnect = mode === 'tiled' ? !!selectedServerUri : !!grantedRoot && selectedFolder !== null;
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') { setGrantedRoot(rootInput.trim()); setBrowseDir(''); setSelectedFolder(null); } }}
              ...
              />
              <button
                onClick={() => { setGrantedRoot(rootInput.trim()); setBrowseDir(''); setSelectedFolder(null); }}
              ...
              <>
                <div className="flex flex-wrap items-center gap-1 text-sm">
                  <button
                    ...
                    onClick={() => { setBrowseDir(''); setSelectedFolder(null); }}
                  >
                    {grantedRoot}
                  </button>
                  {browseDir.split('/').filter(Boolean).map((seg, i, arr) => {
                    ...
                    return (
                      <span key={target} className="flex items-center gap-1">
                        ...
                        <button
                          className="px-2 py-0.5 rounded hover:bg-white/10 text-sky-200"
                          onClick={() => { setBrowseDir(target); setSelectedFolder(null); }}
                        >
                          {seg}
                        </button>
                      </span>
                ...
                <div className="border border-white/20 rounded-md max-h-64 overflow-y-auto text-sm bg-white/5">
                  ...
                  {browseDir && (
                    <button
                      ...
                      onClick={() => { setBrowseDir(browseDir.split('/').slice(0, -1).join('/')); setSelectedFolder(null); }}
                    >
                      ↑ up one level
                    </button>
                  )}
                    ...
                    localEntries
                      ...
                      .map((e) => (
                        <button
                          key={e.path}
                          className="w-full flex items-center gap-2 px-3 py-2 border-b border-white/10 last:border-b-0 hover:bg-white/10 text-sky-100"
                          onClick={() => { setBrowseDir(e.path); setSelectedFolder(null); }}
                        >
                          <Folder size={14} className="text-sky-400 shrink-0" />
                          <span className="font-mono truncate">{e.name}</span>
                        </button>
