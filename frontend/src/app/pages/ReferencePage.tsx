...
export default function ReferencePage() {
  ...
  const sourceKey = source && kind
    ...
    : null;
  const { status: guideStatus, error: guideError, retry: retryGuide } = useGuideSync(sourceKey);
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-3xl flex-col gap-4 overflow-y-auto p-6">
      ...
      {guideStatus === 'loading' && (
        <div className="flex items-center gap-2 rounded-md border border-sky-200 bg-sky-50 px-3 py-2 text-sm text-sky-700">
          <CircleDashed size={16} className="animate-spin" /> Loading this dataset's guide…
        </div>
      )}
      {guideStatus === 'error' && (
        <div className="flex items-center justify-between gap-3 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
          <span className="flex items-center gap-2">
            <WarningCircle size={16} /> {guideError ?? 'Could not load this dataset guide. Edits are not being saved.'}
          </span>
          <button type="button" onClick={retryGuide} className="rounded bg-red-100 px-2 py-1 font-medium hover:bg-red-200">
            Retry
          </button>
        </div>
      )}
